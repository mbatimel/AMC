...
**From the final whole-branch review (commit range `11fa0ce..a920fb6`) — all Critical/Important items were fixed before merge; these Minor/Recommendation items were deliberately deferred:**

- `NEXT_PUBLIC_SITE_ORIGIN` in `deploy/docker-compose.yml` is dead at runtime — Next.js inlines `NEXT_PUBLIC_*` vars at `next build` time, not read from the container's environment. Either wire it as a Docker build `ARG`/CI build-arg, or drop it from compose with a comment noting it's baked in at image build time.
- `PORTAL_API_PROXY_TARGET` in `admin-front`'s env is set to the public `https://wk.amctechgroup.ru` instead of the internal `http://front:3000` — works today only because nginx intercepts `/portal-api/*` before it reaches the container; the internal address would be strictly more robust and avoid an unnecessary hairpin through the public edge.
- `deploy/nginx/conf.d/admin.wk.amctechgroup.ru.conf` proxies `/api/v1/access/`, `/api/v1/auth/`, `/api/v1/users/`, none of which `admin-front` actually calls (only `/api/v1/admin/*`, `/api/v1/products*`, `/api/v1/categories`, `/api/v1/brands` are used) — no new exposure since all three are already public on `wk.amctechgroup.ru` too, but worth trimming for least-privilege.
- `admin-front/yarn.lock` has a hand-patched `@react-aria/utils` entry (added by one of the externally-landed commits, working around a dependency gap in `@heroui/react@3.2.3` that the real published package.json doesn't declare) — durable fix is adding `@react-aria/utils` as an explicit dependency in `admin-front/package.json` so it survives a fresh `yarn install` re-resolution.
- `front`/`admin-front` now run different HeroUI/Next/Tailwind minor versions (duplication strategy side effect — a fresh `yarn install` on each pulls independently) — already caused the `@react-aria/utils` patch above. Worth a README note so future maintainers aren't surprised the two apps' shared-by-copy UI kit can drift.
- `front/src/app/portal-api/README.md` documents the eventual deletion path but doesn't mention that `admin-front` is now a second consumer (via nginx's `/portal-api/` location) — update when convenient.
- Both `front` and `admin-front` default `yarn dev` to port 3000, so they can't run side by side locally without `-p <port>` on one of them.
- `front/src/core/shared/api/signupRequests.ts` (the `front` copy) has two now-dead exports (`listSignupRequests`, `decideSignupRequest`) since only `admin-front` calls those — symmetric with `admin-front`'s copy having a dead `createSignupRequest`. Cost of the duplication strategy; both disappear together once `portal-api` is eventually replaced by a real `back/admin` and this whole temporary layer is deleted per the point above.
- **Pre-existing, not introduced by this split, but worth tracking before `admin.wk.amctechgroup.ru` goes live for real:** `front/src/app/portal-api/**` route handlers have no authn/authz at all (verified — no `X-User-Id`/cookie/session check anywhere in them), and the `admin_user_id` cookie is JS-written, non-`HttpOnly`, non-`Secure`, with the proxy only checking *presence*, not validity. This was already true when `portal-api` lived inside `front`; the split doesn't worsen it, but the whole admin content-editing data layer being unauthenticated is a known risk.
- `admin-front`'s remaining 13 `yarn lint` errors / 5 warnings were individually re-verified (twice, by two different reviewers) as pre-existing debt inherited unchanged from `front`'s original admin code — same standard as `front`'s own ~24 pre-existing lint errors, both out of this plan's scope.
