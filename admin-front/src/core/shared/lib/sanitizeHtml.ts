  ...
  return withoutDanger.replace(
    ...
    (match, tagName: string, attrs: string) => {
      ...
      if (tag === 'span') {
        ...
        // span без разрешённого class не нужен — снимаем обёртку
...
/** Пустой документ TipTap → пустая строка для стора. */
export const normalizeRichTextOutput = (html: string): string => {
  const clean = sanitizeHtml(html).trim();

  if (!clean || /^<p>(?:\s|&nbsp;|<br\s*\/?>)*<\/p>$/i.test(clean)) {
    return '';
  }
