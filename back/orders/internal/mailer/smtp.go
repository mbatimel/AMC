// Package mailer contains the SMTP delivery adapter used for order
// notifications (e.g. order confirmation emails).
package mailer
...
// SendHTML sends an HTML-only email (e.g. an order confirmation with a
// rendered item table and product images referenced by public URL).
func (m *SMTPMailer) SendHTML(ctx context.Context, to string, subject string, html string) error {
	if err := validateHeaders(m.from, to, subject); err != nil {
		return err
	}
	return m.send(ctx, to, buildHTMLMessage(m.from, to, subject, html))
}
...
func buildHTMLMessage(from string, to string, subject string, html string) []byte {
	var b bytes.Buffer
	writeCommonHeaders(&b, from, to, subject)
