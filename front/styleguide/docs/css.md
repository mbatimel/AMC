...
Проект использует **HeroUI v3** (`@heroui/react`) как основную библиотеку UI-компонентов. Библиотека построена на Tailwind CSS v4 — он подключён в сборке (`@import "tailwindcss"` и `@import "@heroui/styles"` в `src/core/shared/styles/index.css`).

### Обязательно: переиспользуй HeroUI

При реализации любого UI **сначала проверяй, есть ли подходящий компонент в HeroUI**, и используй его:

- кнопки → `Button`
- поля ввода → `Input` / `TextField`
- селекты и меню → `Select` / `Dropdown`
- модалки, табы, чекбоксы, свитчи, бейджи и т.д. — аналогично из `@heroui/react`

Свой `<button>`, `<input>` или полностью кастомный контрол допустим **только если**:

- в HeroUI нет подходящего примитива, или
- нужное поведение / a11y / составной паттерн библиотекой не закрывается.

Не дублируй HeroUI «с нуля» ради визуальной близости к макету — кастомизируй через пропсы и CSS Modules.
